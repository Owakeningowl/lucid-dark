...
- **Trainer Sheet** (`src/trainer_browser.html`) — every trainer in fight order, one fight at a time,
  ...
  `pair: "Double"` marks double battles. Link to a split with `#roxanne`, `#elite-four`, etc.
  The old `src/*_split.html` pages forward to their split here.
